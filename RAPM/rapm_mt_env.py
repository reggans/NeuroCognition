...
def _load_rapm_image_dataset(
    ...
    for i, q in enumerate(questions):
        ...
        answer = correct_idx + 1 if isinstance(correct_idx, int) and correct_idx >= 0 else -1
...
State your final answer as: <answer>number</answer>"""
            ...
            rows.append({
                ...
                "info": {
                    ...
                    "max_turns": len(options) if options else 8,  # Use number of options as max turns
                    "mode": "text",
                    "answer_mode": answer_mode,
                    "correct_answer": answer if answer else (correct_idx + 1 if correct_idx is not None else None),
                },
            })
    return Dataset.from_list(rows)
...
class RAPMMultiTurnEnv(MultiTurnEnv):
    ...
    def _get_state_key(self, messages: List[Dict[str, str]], info: Dict[str, Any] = None) -> str:
        ...
        assistant_count = sum(1 for m in subsequent_msgs if m.get('role') == 'assistant')
    ...
    def _get_state_for_messages(self, messages: List[Dict[str, str]], info: Dict[str, Any] = None) -> Dict[str, Any]:
                ...
                if assistant_count == 2 and user_count == 1:
                    ...
                    for stored_key, stored_state in state_snapshot:
                        if stored_key.startswith(f"rapm_{prompt_key}_id_"):
                            if stored_state.get('_first_response') == first_assistant_content:
                    ...
                    if len(assistant_indices) >= 2:
                        ...
                        all_contents = tuple(m.get('content', '') for m in prev_subsequent)
        ...
        _GLOBAL_EPISODE_STATES[key] = {
            ...
            "_first_response": next((m.get('content', '') for m in messages if m.get('role') == 'assistant'), None),
        }
        return _GLOBAL_EPISODE_STATES[key]
    ...
    def env_response(self, messages: List[Dict[str, str]], **kwargs) -> Dict[str, str]:
        ...
        if not last_assistant:
            return {"role": "user", "content": "Please provide your answer in <answer> tags."}
        ...
        if state["turn_idx"] > max_turns:
            return {"role": "user", "content": f"Maximum attempts reached. The correct answer was '{correct}'. RAPM completed.{no_think_suffix}"}
        
        if status != "valid" or choice is None:
            if answer_mode == "gen":
                return {"role": "user", "content": f"Invalid format. Please provide your answer inside <answer> tags. ({max_turns - state['turn_idx']} attempts remaining){no_think_suffix}"}
            else:
                return {"role": "user", "content": f"Invalid format. Please answer with a number 1-{max_options} in <answer> tags. ({max_turns - state['turn_idx']} attempts remaining){no_think_suffix}"}
        ...
        if is_correct:
            return {"role": "user", "content": f"Correct! Your answer '{choice}' is right. RAPM completed.{no_think_suffix}"}
        
        if state["turn_idx"] >= max_turns:
            return {"role": "user", "content": f"Maximum attempts reached. The correct answer was '{correct}'. RAPM completed.{no_think_suffix}"}
        
        return {"role": "user", "content": f"Incorrect. Try again. ({max_turns - state['turn_idx']} attempts remaining){no_think_suffix}"}
            ...
            for msg in completion:
                ...
                if status != "valid":
                    ...
                    if answer_mode == "gen":
                        ...
                        if str(choice).strip().lower() == str(correct_answer).strip().lower():
    ...
    **kwargs,
    ...
    if mode == "image":
        ...
        dataset = _load_rapm_image_dataset(
            eval_data, image_base_path, limit or num_episodes, enable_thinking=enable_thinking
        )
    else:
        if not eval_data:
            # Use default text data path from rapm_data
            eval_data = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "rapm_data", "text_rapm_train.jsonl"
            )
        dataset = _load_rapm_text_dataset(
            eval_data, limit or num_episodes, answer_mode=answer_mode, enable_thinking=enable_thinking
        )
