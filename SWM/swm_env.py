...
class SWMEnv(CognitiveEnv):
    ...
    def __init__(
        ...
        self._last_chosen_coord: Optional[Tuple[int, int]] = (
            None  # For image mode feedback
        )
        self._trial_idx = 0  # Counter for image filenames
        self._needs_image_reset = (
            False  # Show token-in-box frame for one turn before reset
        )
...
Your final answer should be a box number, wrapped with <answer> and </answer>"""
    ...
    def step(self, action: str) -> StepResult:
        ...
        # If previous turn showed a found-token frame, reset to clean board now.
        if self.mode == "image" and self._needs_image_reset:
        ...
        step_info: Dict[str, Any] = {
            ...
            "token_boxes": {
                ...
                if state.token_box[t] is not None
            },
        ...
        if status == ActionStatus.INVALID_FORMAT:
            if self.mode == "image":
                ...
                self._feedback = (
                    f"Please answer with a box number (1-{self.n_boxes}).\n"
                )
        ...
        if status == ActionStatus.INVALID_ACTION:
            self._feedback = (
                f"Please answer with a valid box number (1-{self.n_boxes}).\n"
            )
            self._stats["invalid"] += 1
        ...
        if status == ActionStatus.NOBOX:
            coord_str = (
                str(self._last_chosen_coord) if self._last_chosen_coord else "given"
            )
        ...
        for token in state.tokens:
            if (
                state.token_box[token] is not None
                and chosen_box == state.token_box[token]
            ):
        ...
        if found_tokens:
            ...
            for token in found_tokens:
                self._feedback = (
                    f"Token {token} found in box {box_desc}.\n" + self._feedback
                )
            reward += self.reward_token_found  # Bonus for finding token

            # Preserve a visual frame showing the opened box and found token(s)
            # for this observation before resetting board on the next step.
            if (
                self.mode == "image"
                and self._swm_image is not None
                and self._last_chosen_coord is not None
            ):
                image = self._swm_image.open_box(
                    self._last_chosen_coord, state.token_box
                )
                if self.image_path:
    ...
    def _get_internal_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "legal_boxes": (
                {k: list(v) for k, v in state.legal_boxes.items()} if state else {}
            ),
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if valid_guesses > 0:
            error_rate = (
                self._stats.get("illegal", 0) + self._stats.get("repeated", 0)
            ) / valid_guesses
        ...
        return {
            ...
            "completion_rate": (
                tokens_found / tokens_to_find if tokens_to_find > 0 else 0.0
            ),
