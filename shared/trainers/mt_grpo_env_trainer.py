...
class MTGRPOEnvTrainer(GRPOEnvTrainer):
    ...
    def _generate_and_score_completions(
        ...
        result_segment_indices = self._find_result_positions(
            completion_ids, completion_messages
        )
            ...
            if isinstance(messages, list):
                for j, msg in enumerate(messages):
                    if msg.get("role") == "assistant":
                        if (
                            j + 1 < len(messages)
                            and messages[j + 1].get("role") == "user"
                        ):
    ...
    def _assign_advantages(
        ...
        for i in range(batch_size):
            ...
            outcome_adv_expanded = outcome_adv * torch.ones_like(
                mask_row, dtype=torch.float32
            )
            turn_adv_expanded = turn_adv * torch.ones_like(
                mask_row, dtype=torch.float32
            )
            combined_adv_expanded = combined_adv * torch.ones_like(
                mask_row, dtype=torch.float32
            )
            ...
            if result_segment > 0:
                ...
                if result_segment < len(segment_boundaries):
                    ...
                    before_result_mask = (
                        torch.arange(seq_len, device=device) < split_point
                    ).float() * mask_row
                    assigned_advantages[i] = (
                        outcome_adv_expanded
                        + self.turn_advantage_coef
                        * turn_adv_expanded
                        * before_result_mask
                    )
                else:
